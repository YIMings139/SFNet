...
class Conv3x3(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, use_refl=True):
        ...
        if use_refl:
            self.pad = nn.ReflectionPad2d(1)
            # 反射填充（Reflection Padding）是一种填充输入图像的方法，其目的是在卷积操作之前保持图像的边缘特征不变。
            # 这种填充方式将输入图像的边缘像素值进行反射，使卷积核可以正常作用于图像的边缘区域，从而避免了在卷积过程中丢失边缘信息。
        else:
            self.pad = nn.ZeroPad2d(1)
        self.conv = nn.Conv2d(int(in_channels), int(out_channels), 3)
...
def get_smooth_loss(disp, img):
    ...
    grad_disp_x = F.pad(grad_disp_x, (1, 0))
    grad_disp_y = torch.abs(disp[:, :, :-1, :] - disp[:, :, 1:, :])
    grad_disp_y = F.pad(grad_disp_y, (0, 0, 1, 0))
    ...
    grad_img_x = ((grad_l.abs() + grad_r.abs()).sum(1, keepdim=True) / 2)  # .to(grad_disp_x.dtype)
    grad_img_y = ((grad_t.abs() + grad_b.abs()).sum(1, keepdim=True) / 2)  # .to(grad_disp_x.dtype)
