...
class Trainer:
    def __init__(self, options):
        ...
        self.models["encoder"] = networks.SFNet(model=self.opt.model,
        ...
        if self.use_pose_net:  # 给定姿态网络的优化器。
        ...
        for scale in self.opt.scales:
            ...
            self.backproject_depth[scale] = BackprojectDepth(self.opt.batch_size, h, w)  # 将深度图转换为点云图
            self.backproject_depth[scale].to(self.device)

            self.project_3d[scale] = Project3D(self.opt.batch_size, h, w)  # 将3D图转换为当前相机姿态的像素图
    ...
    def log_time(self, batch_idx, duration, loss):
        ...
        # with open(os.path.join(models_dir, 'train_{}_info.txt'.format(self.opt.model_name)), 'a+') as f:
        #     f.write(print_string.format(self.epoch, self.model_optimizer.state_dict()['param_groups'][0]['lr'],
        #                                 self.model_pose_optimizer.state_dict()['param_groups'][0]['lr'],
        #                                 batch_idx, samples_per_sec, loss,
        #                                 sec_to_hm_str(time_sofar), sec_to_hm_str(training_time_left)) + '\n')
