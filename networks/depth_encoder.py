...
class PFIU(nn.Module):
    def __init__(self, dim, expan_ratio=6, layer_scale_init_value=1e-6, drop_path=0.2):
        ...
        self.bn_master = nn.BatchNorm2d(int(dim / 4))

        self.spa_1 = SpatialAttention(kernel_size=7)
        self.spa_2 = SpatialAttention(kernel_size=5)
        self.spa_3 = SpatialAttention(kernel_size=3)
        self.cha = channelAttention()
        self.conv_gpb = nn.Conv2d(dim, dim, kernel_size=1, bias=False)

        self.bn = nn.BatchNorm2d(dim)

        self.conv7x7_1 = nn.Conv2d(dim, channels_mid, kernel_size=(7, 7), stride=1, padding=3, bias=False)
        self.bn1_1 = nn.BatchNorm2d(channels_mid)
        self.conv5x5_1 = nn.Conv2d(channels_mid, channels_mid, kernel_size=(5, 5), stride=1, padding=2, bias=False)
        self.bn2_1 = nn.BatchNorm2d(channels_mid)
        self.conv3x3_1 = nn.Conv2d(channels_mid, channels_mid, kernel_size=(3, 3), stride=1, padding=1, bias=False)
        ...
        self.pwconv1 = nn.Linear(dim, dim * expan_ratio)
        self.pwconv2 = nn.Linear(dim * expan_ratio, dim)
        self.gelu = nn.GELU()
        self.gamma = nn.Parameter(layer_scale_init_value * torch.ones(dim),
                                  requires_grad=True) if layer_scale_init_value > 0 else None
        self.drop_path = DropPath(drop_path) if drop_path > 0. else nn.Identity()
    ...
    def forward(self, x):
        ...
        x3_1 = self.gelu(x3_1)
        x3_1 = self.spa_3(self.cha(x3_1))
        x3_2 = self.conv3x3_2(x3_1)
        x3_2 = self.bn3_2(x3_2)

        x_out = torch.cat([x1_2, x2_2, x3_2, x_master], dim=1)
